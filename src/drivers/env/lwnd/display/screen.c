...
lwnd_screen_t* create_lwnd_screen(video_device_t* vdev, fb_info_t* fb)
{
    ...
    if (ret->background_window->this_fb && ret->background_window->this_fb->kernel_addr) {
        ...
        void* fb_buf = (void*)ret->background_window->this_fb->kernel_addr;

        for (u64 i = 0; i < ret->px_height; i++) {
            fb_color_t color = { .components = { .r = 0, .g = 0x2f, .b = 0xAf, .a = 0xff } };

            for (u32 j = 0; j < ret->px_width; j++) {
                *(uint32_t volatile*)(fb_buf + j * (fb->bpp >> 3)) = color.raw_clr;

                color.components.r = (j * 255) / ret->px_width;
            }
