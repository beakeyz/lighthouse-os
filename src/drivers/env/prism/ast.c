...
{
    return (
        (current->type == PRISM_TOKEN_TYPE_KEYWORD && next->type == PRISM_TOKEN_TYPE_IDENTIFIER) || (current->type == PRISM_TOKEN_TYPE_IDENTIFIER && (next->type == PRISM_TOKEN_TYPE_ENCAPSULATOR || next->type == PRISM_TOKEN_TYPE_OPERATOR)) || (current->type == PRISM_TOKEN_TYPE_ENCAPSULATOR && (next->type == PRISM_TOKEN_TYPE_ENCAPSULATOR || next->type == PRISM_TOKEN_TYPE_IDENTIFIER)));
}
