...
bool main_menu(gamedata_t* data)
{
    ...
    if (data->games_passed == 0) {
        ...
        if (resp)
            memcpy((void*)data->one_p->name_p, resp, strlen(resp) + 1);
        else
        ...
        if (resp)
            memcpy((void*)data->two_p->name_p, resp, strlen(resp) + 1);
        else
            printf("Could not take in that name!\n");
    ...
    while (true) {
        ...
        while (true) {
            ...
            for (uint8_t i = 0; i < 3; i++) {
                ...
                /* Vertical check */
                if ((data_p->board[0][i] + data_p->board[1][i] + data_p->board[2][i]) == (3 * cur_char))
                    return current;
            }
