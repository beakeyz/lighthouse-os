...
static uint32_t* volatile s_fb_addr;
...
static int __set_mb_tag(struct multiboot_tag_framebuffer* tag)
{
  ...
  s_fb_tag = tag;
  s_fb_addr = (void*)tag->common.framebuffer_addr;
  ...
  s_size = (s_pitch * s_height);

  size_t used_pages = GET_PAGECOUNT(s_size); 

  /* Mark framebuffer memory as used, just to be sure */
  for (uintptr_t i = 0; i < used_pages; i++) {
    kmem_set_phys_page_used(kmem_get_page_idx((paddr_t)s_fb_addr) + i);
  }
...
uintptr_t fb_driver_on_packet(packet_payload_t payload, packet_response_t** response) {
  ...
  switch (payload.m_code) {
    case FB_DRV_MAP_FB: {
      ...
      print("Size: ");
      println(to_string(s_size));
      //kmem_map_range(nullptr, virtual_base, s_fb_addr, s_used_pages, KMEM_CUSTOMFLAG_GET_MAKE | KMEM_CUSTOMFLAG_NO_PHYS_REALIGN, 0);
      __kmem_alloc_ex(nullptr, nullptr, (paddr_t)s_fb_addr, virtual_base, s_size, KMEM_CUSTOMFLAG_NO_REMAP, 0);
      return 0;
      break;
    }
    case FB_DRV_SET_FB:
