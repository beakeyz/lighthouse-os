...
static int __add_pci_driver(pci_driver_t* driver)
{
    ...
    *slot = kzalloc(sizeof(struct pci_driver_link));
     ...
     * NOTE: we ignore the
     */
    if (!__find_fitting_pci_devices(driver) && (driver->device_flags & PCI_DEV_FLAG_VOLATILE_RESCAN) == PCI_DEV_FLAG_VOLATILE_RESCAN)
        return -1;
...
static int _pcidev_get_devinfo(device_t* device, drv_manifest_t* driver, u64 offset, DEVINFO* binfo, size_t bsize)
{
    ...
    binfo->class = pdev->class;
    binfo->subclass = pdev->subclass;
    binfo->deviceid = pdev->dev_id;
    binfo->vendorid = pdev->vendor_id;
