...
static inline uint32_t __keygen_get_seed()
{
 ...
 */
static inline uint32_t __keygen_scramble(uint32_t key)
{
  key *= 0xcc9e2d51;
  key = (key << 15) | (key >> 17);
  key *= 0x1b873593;
 ...
 */
uint32_t __compute_hook_key(const char* hook_name)
{
  size_t len = strlen(hook_name);
  uint32_t h = __keygen_get_seed();
  uint32_t k = 0;
