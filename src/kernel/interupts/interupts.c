...
#define EXEPTION(number, error)           \
static void __exeption_##number() {       \
  kernel_panic(error);                    \
}                                         \
...
EXEPTION(2, "Unknown error (non-maskable interrupt)");

// TODO: breakpoint user-entry

EXEPTION(4, "Overflow");
EXEPTION(5, "Bounds range exceeded");
...
EXEPTION(8, "Double fault");
EXEPTION(9, "Segment overrun");
EXEPTION(10, "Invalid TSS");
EXEPTION(11, "Segment not present");
EXEPTION(12, "Stack segment fault");
...
EXEPTION(15, "Unknown error (Reserved)");
EXEPTION(16, "X87 fp exeption");

static void __unimplemented_exeption() {
  kernel_panic("Unimplemented exeption");
}
...
void init_interupts() {
  ...
  register_idt_interrupt_handler(0x01, __unimplemented_exeption);
  register_idt_interrupt_handler(0x02, __exeption_2);
  register_idt_interrupt_handler(0x03, __unimplemented_exeption);
  register_idt_interrupt_handler(0x04, __exeption_4);
  register_idt_interrupt_handler(0x05, __exeption_5);
  register_idt_interrupt_handler(0x06, illegal_instruction_asm_entry);
  register_idt_interrupt_handler(0x07, fpu_asm_entry);
  register_idt_interrupt_handler(0x08, __exeption_8);
  register_idt_interrupt_handler(0x09, __exeption_9);
  register_idt_interrupt_handler(0x0a, __exeption_10);
  register_idt_interrupt_handler(0x0b, __exeption_11);
  register_idt_interrupt_handler(0x0c, __exeption_12);
  register_idt_interrupt_handler(0x0d, general_protection_asm_entry);
  register_idt_interrupt_handler(0x0e, pagefault_asm_entry);
  register_idt_interrupt_handler(0x0f, __exeption_15);
  register_idt_interrupt_handler(0x10, __exeption_16);

  for (uint8_t i = 0x11; i < 0x20; i++) {
    register_idt_interrupt_handler(i, __unimplemented_exeption);
  }
